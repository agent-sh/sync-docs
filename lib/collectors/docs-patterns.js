...
function analyzeDocIssues(docPath, changedFile, options = {}) {
  ...
  // this doc no longer exists" with rename/deletion signals. Lift those
  // findings rather than regressing to regex + git-show.
 ...
 */
function findDocumentsDeadCode(orphanExports, allDocs, basePath) {
  ...
  for (const orphan of orphanExports) {
    const name = orphan.reason?.match(/`([^`]+)`/)?.[1] || orphan.name;
    if (!name) continue;
 ...
 */
function findDocumentsWrapper(passthroughWrappers, allDocs, basePath) {
  ...
  for (const wrapper of passthroughWrappers) {
    // reason format: "Rust `fn get_user(…)` is a single-call passthrough to `fetch_user` with identical args"
    const [, wrapperName] = wrapper.reason?.match(/`(?:fn |function |def |func )?([A-Za-z_][\w.]*)/) || [];
...
function collect(options = {}) {
  ...
  const documentsDeadCode = analyzer?.orphanExports
    ? findDocumentsDeadCode(analyzer.orphanExports, allDocs, opts.cwd)
    : [];
  const documentsWrapper = analyzer?.passthroughWrappers
    ? findDocumentsWrapper(analyzer.passthroughWrappers, allDocs, opts.cwd)
    : [];
